...
HostDeviceVector<T>::HostDeviceVector()
  : impl_(nullptr) {
  impl_ = new HostDeviceVectorImpl<T>(0, T{}, DeviceOrd::CPU());
}

template <typename T>
HostDeviceVector<T>::HostDeviceVector(Context const*, size_t size, T v, DeviceOrd device)
  : impl_(nullptr) {
  impl_ = new HostDeviceVectorImpl<T>(size, v, device);
}

template <typename T>
HostDeviceVector<T>::HostDeviceVector(Context const*, std::initializer_list<T> init,
                                       DeviceOrd device)
  : impl_(nullptr) {
...
template <typename T>
HostDeviceVector<T>::HostDeviceVector(Context const*, const std::vector<T>& init, DeviceOrd device)
  : impl_(nullptr) {
  impl_ = new HostDeviceVectorImpl<T>(init, device);
}
