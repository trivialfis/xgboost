...
void test_comp() {
  ...
  {
    thrust::device_vector<std::uint8_t> a(h_in.size());
    dh::safe_cuda(cudaMemcpy(a.data().get(), h_in.data(), h_in.size(), cudaMemcpyDefault));
  }
  ...
  dh::DeviceUVector<CompressedByteT> dout(128);
