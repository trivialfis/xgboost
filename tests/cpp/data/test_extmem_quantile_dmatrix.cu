...
auto AssertEllpackEq(Context const* ctx, EllpackPageImpl const* lhs, EllpackPageImpl const* rhs) {
  ...
  auto h_acc = lhs->GetHostEllpack(ctx, &h_buf);
...
class EllpackHostCacheTest : public ::testing::TestWithParam<std::tuple<double, bool, float, int>> {
 public:
  ...
  void Run(float sparsity, bool is_concat, float cache_host_ratio, int n_batches) {
    auto ctx = MakeCUDACtx(0);
    ...
    // Use the same quantile cuts so the bin indices can be compared.
