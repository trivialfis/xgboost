...
class DenseColumn: public Column<BinIdxType> {
 public:
  DenseColumn(ColumnType type, common::Span<const BinIdxType> index,
              uint32_t index_base, size_t feature_offset)
      : Column<BinIdxType>(type, index, index_base),
        feature_offset_(feature_offset) {}
  bool IsMissing(size_t idx) const { return false; }
...
class ColumnMatrix {
 public:
  ...
  inline void Init(const GHistIndexMatrix &gmat, double sparse_threshold) {
...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  inline void SetIndex(uint32_t* index, const GHistIndexMatrix& gmat,
                       const size_t nrow, const size_t nfeature) {
    std::vector<std::pair<bst_row_t, T>> built_index;
    std::vector<bst_row_t> col_ptr;
    common::ParallelGroupBuilder<std::pair<bst_row_t, T>> builder(&col_ptr, &built_index);
    auto* local_index = reinterpret_cast<T*>(&index_[0]);
    const int nthread = omp_get_max_threads();
    builder.InitBudget(gmat.p_fmat->Info().num_col_, nthread);
    // FIXME: External memory.
    for (const auto &page : gmat.p_fmat->GetBatches<SparsePage>()) {
      auto batch = page.GetView();
      ParallelFor(batch.Size(), omp_get_max_threads(), [&](size_t i) {
        int tid = omp_get_thread_num();
        auto row = batch[i];
        CHECK(row.size() == gmat.row_ptr[i + 1] - gmat.row_ptr[i]);
        for (auto const& e : row) {
          builder.AddBudget(e.index, tid);
        }
      });
      builder.InitThreadStorage();

      ParallelFor(batch.Size(), omp_get_max_threads(), [&](size_t i) {
        int tid = omp_get_thread_num();
        auto offset = batch.offset[i];
        for (size_t j = 0; j < gmat.row_ptr[i + 1] - gmat.row_ptr[i]; ++j) {
          auto fidx = batch.data[offset + j].index;
          builder.Transform(fidx, page.base_rowid + i, tid, &row_ind_);
          builder.Transform(fidx, gmat.index[offset + j] - index_base_[fidx], tid, &local_index);
          builder.Increase(fidx, tid);
        }
      });
    }
  }
