...
class TreeEvaluator {
 ...
 public:
  ...
  struct SplitEvaluator {
    ...
    template <typename GradientSumT, typename T = typename GradientSumT::ValueT>
    XGBOOST_DEVICE T CalcSplitGain(const ParamT& param, bst_node_t nidx, bst_feature_t fidx,
      ...
      T wleft = this->CalcWeight(nidx, param, left);
      T wright = this->CalcWeight(nidx, param, right);

      T gain = this->CalcGainGivenWeight(param, left, wleft) +
    ...
    template <typename GradientSumT, typename T = typename GradientSumT::ValueT>
    XGBOOST_DEVICE T CalcWeight(bst_node_t nodeid, const ParamT& param,
                                GradientSumT const& stats) const {
      T w = ::xgboost::tree::CalcWeight(param, stats);
      if (!has_constraint) {
        return w;
      }
    ...
    template <typename GradientSumT, typename T = typename GradientSumT::ValueT>
    XGBOOST_DEVICE T CalcWeightCat(ParamT const& param, GradientSumT const& stats) const {
      static_assert(std::is_floating_point_v<T>);
    ...
    template <typename GradientSumT, typename T = typename GradientSumT::ValueT>
    XGBOOST_DEVICE T CalcGainGivenWeight(ParamT const& p, GradientSumT const& stats, T w) const {
      static_assert(std::is_floating_point_v<T>);
      if (stats.GetHess() <= 0) {
        return .0f;
      }
      ...
      return tree::CalcGainGivenWeight<ParamT, T>(p, stats.GetGrad(), stats.GetHess(), w);
    }
    template <typename GradientSumT, typename T = typename GradientSumT::ValueT>
    XGBOOST_DEVICE T CalcGain(bst_node_t nid, ParamT const& p, GradientSumT const& stats) const {
