...
__device__ void EvaluateFeature(
    bst_feature_t fidx, EvaluateSplitInputs<GradientSumT> inputs,
...
__global__ void EvaluateSplitsKernel(
  ...
  if (common::IsCat(inputs.feature_types, fidx)) {
    EvaluateFeature<BLOCK_THREADS, SumReduceT, BlockScanT, MaxReduceT,
                    ...
                    UpdateOneHot<GradientSumT>>(fidx, inputs, &best_split,
                                                &temp_storage);
  } else {
    EvaluateFeature<BLOCK_THREADS, SumReduceT, BlockScanT, MaxReduceT,
                    TempStorage, GradientSumT,
                    NumericBin<GradientSumT, TempStorage, BlockScanT>,
                    UpdateNumeric<GradientSumT>>(fidx, inputs, &best_split,
                                                 &temp_storage);
  }
