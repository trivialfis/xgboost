 ...
 public:
  ...
  void ParseStat(int32_t n_nodes, std::vector<RTreeNodeStat>* p_stats) {
    ...
    for (int32_t i = 0; i < n_nodes; ++i) {
      ...
      auto& s = stats[i];
      size_t pos {0};
      s.loss_chg = std::stod(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      s.sum_hess = std::stod(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      s.base_weight = std::stod(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      s.leaf_child_cnt = std::stoi(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(']');
      if (i != n_nodes - 1) {
        GetChar(',');
  ...
  void ParseNode(int32_t n_nodes, std::vector<Node>* p_nodes) {
    ...
    for (int32_t i = 0; i < n_nodes; ++i) {
      GetChar('[');
      size_t pos {0};
      auto left = std::stoi(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      auto right = std::stoi(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      auto par = std::stoi(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      auto split = common::stoui(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(',');
      auto cond = std::stof(raw_str_.substr(cursor_.Pos(), 17), &pos);
      ForwardN(pos);
      GetChar(']');
      nodes[i]= {left, right, par, split, cond};
      if (i != n_nodes - 1) {
        GetChar(',');
      }
    }
...
void RegTree::Save(Json* p_out) const {
  ...
  std::string tree_raw;
  tree_raw.reserve(stats_.size() * 9 * 2);
  tree_raw += '[';
  for (size_t i = 0; i < stats_.size(); ++i) {
    auto const& s = stats_[i];
    tree_raw += '[';
    tree_raw += to_string(s.loss_chg) + ",";
    tree_raw += to_string(s.sum_hess) + ",";
    tree_raw += to_string(s.base_weight) + ",";
    tree_raw += to_string(s.leaf_child_cnt);
    tree_raw += ']';
    if (i != stats_.size() - 1) {
      tree_raw += ',';
    }
  ...
  for (size_t i = 0; i < nodes_.size(); ++i) {
    ...
    tree_raw += "[";
    tree_raw += to_string(n.LeftChild()) + ",";
    tree_raw += to_string(n.RightChild()) + ",";
    tree_raw += to_string(n.Parent()) + ",";
    tree_raw += to_string(n.SplitIndex()) + ",";
    tree_raw += to_string(n.SplitCond());
    tree_raw += "]";
    if (i != nodes_.size() - 1) {
      tree_raw += ',';
    }
