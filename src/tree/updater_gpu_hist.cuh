...
class MultiTargetHistMaker {
 private:
  ...
  void BuildHist(EllpackPage const& page, std::int32_t k, bst_node_t nidx) {
    this->BuildHist(page, k, std::vector<bst_node_t>{nidx});
  }

  void BuildHist(EllpackPage const& page, std::int32_t k, std::vector<bst_node_t> const& nodes) {
    ...
    for (auto nidx : nodes) {
      ...
      mean_node_size += d_ridx.size() / nodes.size();
    }
    std::cout << "mean_node_size:" << mean_node_size << std::endl;
    ...
    this->histogram_.BuildHistogram(
        ...
        d_gpair, dh::ToSpan(ridxs), dh::ToSpan(hists), mean_node_size, roundings);
  }
